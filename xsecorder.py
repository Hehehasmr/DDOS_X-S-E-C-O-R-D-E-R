'''
Disclaimer: Dont do this on websites you dont have accses to.
...
try:
    ...
    try:
        ...
        # fallback: basic passthrough if tqdm is not installed
...
by: XSECORDER  
    ...
    def check_con():
        try:
            # Use certifi CA bundle if available to avoid macOS Python certificate verification issues
            if certifi:
                ctx = ssl.create_default_context(cafile=certifi.where())
            else:
                ctx = ssl.create_default_context()

            # Allow bypass via env var for local testing only
            if os.environ.get("XSECORDER_INSECURE") == "1":
                ctx = ssl._create_unverified_context()

            request.urlopen('https://www.google.co.in/', timeout=3, context=ctx)
        except KeyboardInterrupt:
            print(Fore.RED + Style.BRIGHT + "Detenido por el usuario..." + Style.RESET_ALL)
            sys.exit(1)
        except Exception as e:
            # Provide a clearer message including common macOS fix
            extra = ""
            if certifi is None:
                extra = "\n(Install the 'certifi' package or run the 'Install Certificates.command' included with python.org installer on macOS.)"
            print(Fore.RED + Style.BRIGHT + 'Por favor revisa tu conexion a internet... ' + Style.RESET_ALL + str(e) + extra)
            sys.exit(1)
